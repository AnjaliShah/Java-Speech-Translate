...
public class SpeechTranslationSessionSocket 
{
    ...
    public void onClose(int statusCode, String reason) {
        System.out.printf("[%s] Connection closed: %d - %s%n", new Date().toString(), statusCode, reason);
        this.session = null;
    ...
    public void onConnect(Session session) {
        System.out.printf("[%s] Connection Success: %s%n", new Date().toString(), session);
        this.session = session;
        try 
        {
            //start streaming the audio
			sendData(session, inputAudioFileName);
			System.out.printf("%n[%s] Done streaming audio.%n", new Date().toString());
        } 
        catch (Throwable t) 
        {
            t.printStackTrace();
    ...
    public void onMessage(String msg) {
        ...
        System.out.printf("[%s] Message Received: %s%n", new Date().toString(), msg);
    }
    ...
    {	
		try
		{
			try(FileInputStream fi = new FileInputStream(new File(fileName)))
			{
				...
				int counter=0;
	           	System.out.printf("[%s] Sending Bytes: ", new Date().toString());
